...
public abstract class ThriftUtil {
	 ...
	 * @param client
	 */
	public static void openClient(TServiceClient client) {
		if (client == null) {
			return;
		}
		...
		try {
			...
			if (proto != null && !proto.getTransport().isOpen()) {
				proto.getTransport().open();
			}
		} catch (Throwable e) {
			logger.warn("open input transport fail", e);
		}
		try {
			...
			logger.warn("open output transport fail", e);
		}
	}
